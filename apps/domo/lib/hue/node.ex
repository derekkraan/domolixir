defmodule Hue.Node do
  ...
  def start(controller_name, node_id) do
    ...
    case Supervisor.start_child(
           HueBridge.network_supervisor_name(controller_name),
           worker(
             __MODULE__,
  ...
  def request_state(state) do
    ...
    light_info =
      GenServer.call(state.name, {:command, {:light_info, state.node_id}}) |> IO.inspect()
    ...
    %{
      event_type: "brightness_level",
      node_identifier: node_identifier,
      brightness_level: light_info["state"]["bri"] / 255
    }
    |> EventBus.send()
    ...
    %{
      ...
      node_identifier: node_identifier,
      alive: light_info["state"]["reachable"]
    }
    |> EventBus.send()

    %{event_type: "node_name", node_identifier: node_identifier, name: light_info["name"]}
    |> EventBus.send()

    %{event_type: "node_type", node_identifier: node_identifier, type: light_info["type"]}
    |> EventBus.send()
  ...
  def handle_call({:command, {:set_brightness, brightness}}, _from, state) do
    %{status: :ok} =
      GenServer.call(state.name, {:command, {:set_brightness, state.node_id, brightness}})
