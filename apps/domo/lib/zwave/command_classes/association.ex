defmodule ZWave.Association do
  ...
  def add_associations(state) do
    ...
    1..state.number_association_groups
  ...
  def association_set_command(to_node_id, group_id, node_id) do
    %ZWave.Msg{
      type: @request,
      function: @func_id_zw_send_data,
      data: [node_id, 0x04, @command_class, @associationcmd_set, group_id, to_node_id],
      target_node_id: node_id
    }
  end
  ...
  def private_process_message(
        state,
        <<@sof, _length, @request, @func_id_application_command_handler, _callback_id, node_id, 3,
