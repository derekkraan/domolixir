defmodule ZWave.Basic do
  ...
  def handle({:basic_set, level}, node_id) do
    %ZWave.Msg{
      ...
      data: [node_id, 0x03, @command_class, @basic_set, level],
  ...
  def handle({:basic_get}, node_id) do
    %ZWave.Msg{
      type: @request,
      function: @func_id_zw_send_data,
      data: [node_id, 0x02, @command_class, @basic_get],
      target_node_id: node_id
    }
  end
  ...
  def private_process_message(
        ...
        msg =
          <<@sof, _msglength, @request, @func_id_application_command_handler, _status, _node_id,
            _length, @command_class, @basic_set, value, _rest::binary>>
      ) do
    %{
      node_id: node_id,
      name: name,
      event_type: "basic_set",
      data: %{value: if(value > 0, do: 1, else: 0)}
    }
    |> EventBus.send()
  end
end
