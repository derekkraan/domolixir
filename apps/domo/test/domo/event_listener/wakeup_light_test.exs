defmodule WakeupLightTest do
  ...
  setup context do
    {:ok, normal_time_pid} =
      Domo.EventListener.WakeupLight.start_link(%Domo.EventListener.WakeupLight{
        time: [hour: 17, minute: 15],
        days_of_week: ["Fri"],
        light_intensity: 50,
        minutes_to_fade_in: 30,
        node_id: self()
      })

    {:ok, just_after_midnight_pid} =
      Domo.EventListener.WakeupLight.start_link(%Domo.EventListener.WakeupLight{
  ...
  test "doesn't turn on a light if it isn't time yet", context do
    day =
      Timex.now() |> Timex.set(year: 2017, month: 10, day: 27, hour: 14, minute: 0, second: 00)

    send(
      context[:normal_time_pid],
      {:event, %{event_type: "clock_update", node_id: nil, datetime: day}}
    )
  ...
  test "wrong day (Saturday)", context do
    day =
      Timex.now() |> Timex.set(year: 2017, month: 10, day: 28, hour: 14, minute: 0, second: 00)
  ...
  test "turns on a light when the time is approaching", context do
    day =
      Timex.now() |> Timex.set(year: 2017, month: 10, day: 27, hour: 16, minute: 45, second: 00)
  ...
  test "when halfway through fadein time, sets the lamp to 50% of final intensity", context do
    day =
      Timex.now() |> Timex.set(year: 2017, month: 10, day: 27, hour: 17, minute: 00, second: 00)
  ...
  test "fades in per second", context do
    day =
      Timex.now() |> Timex.set(year: 2017, month: 10, day: 27, hour: 16, minute: 45, second: 50)
  ...
  test "works across days", context do
    day =
      Timex.now() |> Timex.set(year: 2017, month: 10, day: 26, hour: 23, minute: 51, second: 00)

    send(
      context[:just_after_midnight_pid],
      {:event, %{event_type: "clock_update", node_id: nil, datetime: day}}
    )
  ...
  test "still checks day of week properly across dates", context do
    day =
      Timex.now() |> Timex.set(year: 2017, month: 10, day: 25, hour: 23, minute: 51, second: 00)
