defmodule ZWave.WakeUp do
  ...
  def process_name(name, node_id),
    do: :"#{ZWave.Node.node_name(name, node_id)}_wake_up_command_class"
  ...
  def wakeup_get_interval_command(node_id) do
    %ZWave.Msg{
      ...
      data: [node_id, 0x02, @command_class, @wakeup_cmd_interval_get, @transmit_options],
  ...
  def wakeup_no_more_information_command(node_id) do
    %ZWave.Msg{
      type: @request,
      function: @func_id_zw_send_data,
      data: [node_id, 0x02, @command_class, @wakeup_cmd_no_more_information, @transmit_options],
      target_node_id: node_id,
      expected_response: @func_id_zw_send_data
    }
  end
  ...
  def wakeup_command_interval_report(node_id) do
    %ZWave.Msg{
      type: @request,
      function: @func_id_zw_send_data,
      data: [node_id, 0x02, @command_class, @wakeup_cmd_interval_report, @transmit_options],
      target_node_id: node_id,
      expected_response: @func_id_application_command_handler
    }
  end
  ...
  def private_process_message(
        state,
        <<@sof, _length, @request, @func_id_application_command_handler, _status, node_id,
          _length2, @command_class, @wakeup_cmd_notification, _checksum>>
      ) do
    %State{state | awake: true} |> IO.inspect()
  end
  ...
  def private_process_message(
        state,
        <<@sof, _length, @response, @func_id_application_command_handler, 0,
          @wakeup_cmd_interval_capabilities_report, _rest::binary>>
      ) do
