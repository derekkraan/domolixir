defmodule ZWave.Msg do
  ...
  def update_backoff_time(command),
    do: %{command | backoff_time: round(command.backoff_time * 1.5)}

  def prepare(
        msg = %ZWave.Msg{type: type, function: function, data: data, callback_id: callback_id}
      ) do
    msg
  ...
  def required_response?(
        %ZWave.Msg{
          ...
          data: [_node_id, _length, @command_class_association, _cmd]
        },
        _resp
      ),
      do: false
  ...
  def required_response?(
        %ZWave.Msg{callback_id: callback_id},
        <<@sof, _length, _req_res, _respons, callback_id, _rest::binary>>
      )
      when not is_nil(callback_id),
      do: true

  def required_response?(%ZWave.Msg{callback_id: callback_id}, _msg) when not is_nil(callback_id),
    do: false

  def required_response?(
        %ZWave.Msg{expected_response: expected_response},
        <<@sof, _length, _req_res, expected_response, _rest::binary>>
      )
      when not is_nil(expected_response),
      do: true

  def required_response?(%ZWave.Msg{expected_response: expected_response}, _)
      when not is_nil(expected_response),
      do: false

  def required_response?(
        %ZWave.Msg{function: @func_id_zw_get_node_protocol_info},
        <<@sof, _length, @response, @func_id_zw_get_node_protocol_info, _rest::binary>>
      ),
      do: true

  def required_response?(%ZWave.Msg{function: @func_id_zw_get_node_protocol_info}, _resp),
    do: false

  def required_response?(
        %ZWave.Msg{function: @func_id_zw_send_data},
        <<@sof, _length, @response, @func_id_zw_send_data, _rest::binary>>
      ),
      do: true
  ...
  def required_response?(
        %ZWave.Msg{function: function},
        <<@sof, _length, @response, function, _rest::binary>>
      ),
      do: true
